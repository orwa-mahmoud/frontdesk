...
def test_embedder_defers_client_creation() -> None:
    embedder = OpenAIEmbedder(api_key="", model=_MODEL, dimensions=_DIMS)
...
def test_embedder_creates_client_with_key() -> None:
    embedder = OpenAIEmbedder(api_key="sk-test-key-12345678", model=_MODEL, dimensions=_DIMS)
    client = embedder._get_client()
    assert client is not None


def test_embedder_dimensions() -> None:
    embedder = OpenAIEmbedder(api_key="sk-x", model=_MODEL, dimensions=768)
    assert embedder.dimensions == 768


@pytest.mark.asyncio
async def test_embed_documents_empty_returns_empty() -> None:
    embedder = OpenAIEmbedder(api_key="sk-x", model=_MODEL, dimensions=_DIMS)
